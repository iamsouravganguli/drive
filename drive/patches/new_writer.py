...
def execute():
    files = bor.get_all("Drive File", filters={"document": ("!=", "")}, fields=["name", "document"])
    yjs_map = {k["name"]: bor.get_doc("Drive Document", k["document"]).content for k in files}

    for name, yjs_data in yjs_map.items():
        drive_file = bor.get_doc("Drive File", name)
        try:
            ...
            for annotation in annotations:
                ...
                for reply in replies:
                    ...
                    reply_doc = bor.get_doc({"doctype": "Drive Comment", "content": content, "name": str(uuid4())})
            ...
            for name, (owner, created_at) in MAP.items():
                bor.db.set_value("Drive Comment", name, "owner", owner)
                bor.db.set_value("Drive Comment", name, "creation", created_at)
            doc = bor.get_doc("Drive Document", drive_file.document)
            ...
            print("ERROR", e)
            bor.log_error(f"Error processing Yjs content: {e}")
